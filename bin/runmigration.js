...
const dialect = sequelize.connectionManager.dialectName;
...
(async function() {
  if (!fromRevision) {
    if (dialect === 'mysql') {
      await sequelize.query('CREATE TABLE IF NOT EXISTS `Migrations` ( `id` INT NOT NULL AUTO_INCREMENT , ' +
        '`rev` INT NOT NULL , `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`), INDEX (`rev`)) ENGINE = InnoDB')
    } else if (dialect === 'postgres') {
      await sequelize.query('CREATE TABLE IF NOT EXISTS Migrations ( id SERIAL PRIMARY KEY, ' +
        'rev INT NOT NULL UNIQUE, createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)')
    } else {
      throw new Error(`Unknown dialect ${ dialect }`)
    }

    let [ lastRevision ] = await sequelize.query(
      'SELECT * FROM Migrations ORDER BY rev DESC LIMIT 1',
      { type: sequelize.QueryTypes.SELECT }
    )
    fromRevision = lastRevision ? lastRevision.rev + 1 : 0
  }
  ...
  let migrationFiles = fs.readdirSync(migrationsDir)
    ...
    // sort by revision
    .sort((a, b) => {
      let revA = parseInt(path.basename(a)
        .split('-', 2)[0]),
        revB = parseInt(path.basename(b)
          .split('-', 2)[0])
    ...
    .filter((file) => {
      let rev = parseInt(path.basename(file)
        .split('-', 2)[0])
  ...
  Async.eachSeries(
    ...
    function(file, cb) {
      ...
      migrate.executeMigration(queryInterface, path.join(migrationsDir, file), fromPos, (err) => {
        ...
        let query;
        if (dialect === 'mysql') {
          query = 'INSERT INTO Migrations (id, rev) VALUES (NULL, ?)';
        }
        else if (dialect === 'postgres') {
          query = 'INSERT INTO Migrations (id, rev) VALUES (DEFAULT, ?)';
        }
        else {
          throw new Error(`Unknown dialect ${dialect}`);
        }
        ...
        sequelize.query(
          query,
          { replacements: [rev], type: sequelize.QueryTypes.INSERT }
          )
          .then(() => {
            if (stop)
              return cb("Stopped");
